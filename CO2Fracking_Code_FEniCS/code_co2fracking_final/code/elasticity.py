...
##############################################################################################################
sigma_R =  Constant([-1.,0.]) #traction on right boundary
sigma_L =  Constant([1.,0.]) #traction on right boundary
sigma_B =  Constant([0., 1.]) #traction on right boundary
sigma_T =  Constant([0.,-1.]) #traction on right boundary
...
if not dispMehtod: 
	Gamma_u_1 = DirichletBC(V_u, zero_v, boundaries, 2)
	Gamma_u_2 = DirichletBC(V_u, zero_v, boundaries, 3) 
	bc_u=[Gamma_u_1, Gamma_u_2];
else:  
...
if not dispMehtod:  
	external_work = dot(body_force, u_)*dx+ dot(sigma_R, u_)*ds(1)+ dot(sigma_T, u_)*ds(4)#+dot(sigma_B, u_)*ds(3)+ dot(sigma_L, u_)*ds(2);
else: 
