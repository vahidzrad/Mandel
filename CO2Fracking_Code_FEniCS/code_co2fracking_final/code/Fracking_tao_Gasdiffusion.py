...
def Fracking(E, nu, hsize, ell, law, ModelB, ka, kb, q, pressure_steps):
	...
	geofile = \
			...
			a=10.;
		        ...
		        Point(10) = {L/2, H-a, 0, 1*lc};
		        Point(11) = {L/2, a, 0, 1*lc};
			...
			Line(10) = {5, 10};
		        Line{10} In Surface{30};

			Line(11) = {8, 11};
		        Line{11} In Surface{30};

			Physical Surface(1) = {30};

			Physical Line(101) = {6};
			Physical Line(102) = {7};
			Physical Line(103) = {10};
			Physical Line(104) = {11};
	...
	DeltaT = hsize**2 *mu_dynamic/(kappa*Rho_intial*N_intial[0])  # non dimensional time (s) 
	...
	prefix_stress = "L%s-H%.2f-%s-%g"%(L,H,'StressBC',hsize)
	...
	#Pressure = rho*div(u_)*P*dx- rho*div(u_0)*P*dx+phi/N*P_*P*dx + DeltaT*(kappa*exp((ka*alpha_)**kb)/mu_dynamic)*rho*inner(nabla_grad(P_), nabla_grad(P))*dx-(phi/N*P_0 +DeltaT*f)*P*dx-rho*DeltaT*Q*P*ds(5)

	#Pressure = rho*div(u_)*P*dx- rho*div(u_0)*P*dx+phi/N*P_*P*dx + DeltaT*(kappa*exp((ka*alpha_)**kb)/mu_dynamic)*rho*inner(nabla_grad(P_), nabla_grad(P))*dx-(phi/N*P_0 +DeltaT*f)*P*dx-rho*DeltaT*Q*P*ds(5)-DeltaT*(kappa*exp((ka*alpha_)**kb)/mu_dynamic)/N*(nabla_grad(P_))**2*P*dx-DeltaT*(1/mu_dynamic)*nabla_grad( kappa*exp((ka*alpha_)**kb) )*nabla_grad(P_)*P*dx

	Pressure = rho*div(u_)*P*dx- rho*div(u_0)*P*dx+phi*rho*P*dx -phi*rho_0*P*dx + DeltaT*(kappa*exp((ka*alpha_)**kb)/mu_dynamic)*rho*inner(nabla_grad(P_), nabla_grad(P))*dx-rho*DeltaT*Q*P*ds(5)
	#------------------------------------------------------------------------------------
	...
	Length = np.zeros((len(load_multipliers),2))
	...
	for (i_p, p) in enumerate(load_multipliers):
	    ...
	    np.savetxt(save_dir+'/pressureboreholes'+prefix+'.txt', Borehole_Pressure)
	    Length[i_p] = np.array([p,assemble( 1/float(c_w)*(w(alpha_)/ell + ell*inner(grad(alpha_), grad(alpha_)))*dx)])
   	    np.savetxt(save_dir+'/length.txt', Length)
	...
	s = sigma(u_,alpha_) - (1./3)*tr(sigma(u_,alpha_))*Identity(ndim)  # deviatoric stress
	von_Mises = sqrt(3./2*inner(s, s))
	V = FunctionSpace(mesh, 'P', 1)
	von_Mises = project(von_Mises, V)
	File(save_dir+"von_Mises.pvd") << von_Mises

	stress_tr = project(tr(sigma(u_,alpha_)), V)
	File(save_dir+"stress_tr.pvd") << stress_tr

	# Project and write stress field to post-processing file
	sigma_x = project( (inner(sigma(u_,alpha_)*e1,e1)), V)
	File(save_dir+"sigma_x.pvd") << sigma_x

	sigma_y = project((inner(sigma(u_,alpha_)*e2,e2)), V)
	File(save_dir+"sigma_y.pvd") << sigma_y
